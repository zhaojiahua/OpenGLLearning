...
//
//	//创建着色器
//	Shader myShader1("shaders/openGLLearning29/vertexShader.vs.c", "shaders/openGLLearning29/fragmentShader1.fs.c");//带几何着色器
...
//			temp_offset.x = float(j) / 10.0f + 0.1f;
//			temp_offset.y = float(i) / 10.0f + 0.1f;
//			to_offsets[index++] = temp_offset;
//		}
//	}
//	//将这100个数据传递给顶点着色器(发送给uniform 的数据量是有上限的,当超过100个数据的时候需要用到实例化数组,看下一节)
//	myShader1.Use();
//	for (unsigned int i = 0; i < 100; i++) {
//		string temp_name1 = "offsets[";
//		string temp_name2 = "]";
//		string temp_name=temp_name1 + to_string(i)+ temp_name2;
//		myShader1.SetVec2f(temp_name, to_offsets[i]);
//	}
//
