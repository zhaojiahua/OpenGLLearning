...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, projectTitle.c_str(), NULL, NULL);
	if (window == nullptr) {
	...
	glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);//设置显示鼠标
	...
	//hdr贴图(等距柱状投影图)
	GLuint hdrmap = testModel_box.meshes[0].TextureFromFile_f("assets/textures/4KHDRIS/2.hdr");
	...
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	////////////////////////////////////////////////////////////////////////////////////开始渲染流程
	glClearColor(0.1f, 0.5f, 0.1f, 0.0f);	//设置颜色缓冲区的颜色值

	//渲染hdr等距柱状纹理图到盒子纹理(这个渲染一定要放在循环外面)
	glBindFramebuffer(GL_FRAMEBUFFER, hdrFBO);
	glViewport(0, 0, 1024, 1024);//把视口分辨率设置1024X1024
	...
	while (!glfwWindowShouldClose(window)) {	//如果检测到GLFW要求被关闭就结束循环
		...
		if (saveImage) {//如果需要保存图片就不渲染天空盒子,直接保存当前帧缓冲到图片文件
			auto now_sys = chrono::system_clock::now();
			auto timestamp_ms = now_sys.time_since_epoch().count();
			SaveFramebufferToImage("RenderOut/" + projectTitle + to_string(timestamp_ms) + ".png", 1920, 1080);	//保存当前帧缓冲到图片文件
			saveImage = false;
		}
		else {//如果不保存图片就渲染天空盒子
