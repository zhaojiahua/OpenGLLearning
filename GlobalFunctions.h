...
#include "stb_image.h"//图片读写头文件
#include "stb_image_write.h"//图片读写头文件
...
//全局变量
string projectTitle = "PBR Render";//项目名称
//设置屏幕宽高
...
//创建一个摄像机
ZCamera zcamera(20.0f, static_cast<float>(screenWidth) / static_cast<float>(screenHeight), 0.1f, 1000.0f);
...
void SaveFramebufferToImage(const std::string& filename, int width, int height) {
	std::vector<unsigned char> pixels(width * height * 4); // RGBA format
	glPixelStorei(GL_PACK_ALIGNMENT, 1); // Ensure proper alignment
	glEnable(GL_BLEND);
...
void ProcessInput(GLFWwindow* inWind) {
	...
	if (glfwGetKey(inWind, GLFW_KEY_Q) == GLFW_PRESS) {//如果检测到Q键被按下
		saveImage = true;
	}
	if (glfwGetKey(inWind, GLFW_KEY_W) == GLFW_PRESS) {//W键按下相机向前运动
		zcamera.MoveForward();
