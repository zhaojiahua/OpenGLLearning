...
//	//cout << testModel.meshes[0].mIndices.size() << endl;
//	//创建顶点着色器
//	Shader myShader1("shaders/openGLLearning20/vertexShader.vs.c", "shaders/openGLLearning20/fragmentShader1.fs.c");
...
//	//配置聚光灯
//	spotLight.SetLight(&myShader1, 0, 0.1f, 0.3f, glm::vec3(1.0f, 1.0f, 1.0f));
//
//
//	//开启深度测试
...
//	glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE);//如果通过模版测试,就把存储的模版值设置为参考值
//
//	//保持渲染循环
...
//		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);	//清除颜色缓冲和深度缓冲和模版缓冲
//
//		//激活这个程序对象
//		myShader1.Use();
...
//		//绘制地板的时候不更新模版缓冲
//		glStencilMask(0x00);
//		testModel_Ground.Draw(&myShader1, tempModel);
//
//		glStencilFunc(GL_ALWAYS, 1, 0xFF);//所有的片段都更新模版缓冲
//		glStencilMask(0xFF);//启用模版写入
//		testModel.Draw(&myShader1, tempModel);
//
//		//绘制边缘
//		glStencilFunc(GL_NOTEQUAL, 1, 0xFF);//模版值不等于1的时候绘制
//		glStencilMask(0x00);//进制模版写入
//		glDisable(GL_DEPTH_TEST);//深度测试关闭
//		myShader1_Single.Use();//使用边缘着色器
//		myShader1_Single.SetVec3f("viewPos", zcamera.cameraPos);
//		glUniformMatrix4fv(glGetUniformLocation(myShader1_Single.ID, "v_matrix"), 1, GL_FALSE, glm::value_ptr(zcamera.GetCameraViewMatrix()));
//		glUniformMatrix4fv(glGetUniformLocation(myShader1_Single.ID, "p_matrix"), 1, GL_FALSE, glm::value_ptr(zcamera.GetCameraperspectiveMatrix()));
//		glm::mat4 scaleMM = glm::mat4(1.0f);
//		scaleMM = glm::scale(scaleMM, glm::vec3(0.82f));
//		testModel.Draw(&myShader1_Single, scaleMM);
//		glStencilMask(0xFF);//模版缓冲写入开启
