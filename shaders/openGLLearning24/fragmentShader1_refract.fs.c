...
//处理平行光的函数
vec4 CalculateDirLight(DirLight light,vec3 normal,vec3 viewDir,vec4 reflectColor,vec4 refractColor){
	vec3 lightDir=normalize(-light.direction);
	float diff=max(dot(lightDir,normal),0.0f);
	vec3 reflectDir=reflect(-lightDir,normal);
	float spec=pow(max(dot(viewDir,reflectDir),0.0f),material.shininess);
	vec4 tempdiffuse=refractColor+2*reflectColor+0.1*(tempdiffuse_+texture(material.texture_diffuse1,vertexUV)+texture(material.texture_diffuse2,vertexUV)+texture(material.texture_diffuse3,vertexUV));
...
//处理点光源的函数
vec4 CalculatePointLight(PointLight light,vec3 normal,vec3 fragPos,vec3 viewDir,vec4 reflectColor){
	...
	vec4 tempdiffuse=reflectColor+texture(material.texture_diffuse1,vertexUV)+texture(material.texture_diffuse2,vertexUV)+texture(material.texture_diffuse3,vertexUV);
	vec4 ambient=light.ambient*tempdiffuse;
	vec4 diffuse=light.diffuse*diff*tempdiffuse;
	vec4 tempspecular=texture(material.texture_specular1,vertexUV)+texture(material.texture_specular2,vertexUV)+texture(material.texture_specular3,vertexUV);
	vec4 specular=light.specular*spec*tempspecular;
	return (ambient+diffuse+specular)*attenuation;
}
//计算聚光灯的函数
vec4 CalculateSpotLight(SpotLight light,vec3 normal,vec3 fragPos,vec3 viewDir,vec4 reflectColor,vec4 refractColor){
	vec3 lightDir=normalize(light.position-fragPos);
...
void main()
{
	...
	vec3 refractDir=refract(-viewDir,norN,refractRatio);
	vec4 refractColor=vec4(texture(cubemap,refractDir).rgb,1.0f);
	vec4 tempresult;
	//首先平行光的计算
	for(int i=0;i<DIRECTOINLIGHT_NUM;i++){
		tempresult+=CalculateDirLight(dirlights[i],norN,viewDir,reflectColor,refractColor);
	}
	//然后点光源计算
	...
	for(int i=0;i<SPOTLIGHT_NUM;i++){
		tempresult+=CalculateSpotLight(spotlights[i],norN,fragPos,viewDir,reflectColor,refractColor);
	}
