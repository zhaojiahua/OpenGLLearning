...
uniform samplerCube texture_irradiance;//辐照度贴图
...
void main()
{
	...
	vec3 outks=FresnelSchlickRoughness(max(dot(N,V),0.0),F0,roughness);
	vec3 outkd=1.0-outks;
	vec3 diffuse=outkd*albedo*irradiance;
	vec3 ambient=diffuse*ao;//给个环境光
